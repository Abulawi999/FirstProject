...
//	hala wallah
	...
	String Password = "@11@Engineering@155@";
	...
	public void signUp() throws InterruptedException {
		...
		int RandomIndexForTheLasttName = rand.nextInt(LastName.length);

		String UserFirstName = firstname[RandomIndexForTheFirstName];
		String UserLastName = LastName[RandomIndexForTheLasttName];

		int randomnumberForTheEmail = rand.nextInt(11299);
		...
		AddressInput.sendKeys("Jabal Al-Qousour Mansour-Bin-Iraq St..");
		...
		WebElement zone_idInput = driver.findElement(By.id("AccountFrm_zone_id"));

		Select selector = new Select(zone_idInput);

		int randomZone = rand.nextInt(1, 6);

		selector.selectByIndex(randomZone);

		WebElement ZipPostalCodeInput = driver.findElement(By.id("AccountFrm_postcode"));
		ZipPostalCodeInput.sendKeys("11299");

		WebElement LoginNameInput = driver.findElement(By.id("AccountFrm_loginname"));

		String LocalLoginName = UserFirstName + UserLastName + randomnumberForTheEmail;
		LoginNameInput.sendKeys(LocalLoginName);

		GlobalUserNameForTheLogin = LocalLoginName;
		...
		WebElement CheckBoxAgree = driver.findElement(By.id("AccountFrm_agree"));
		CheckBoxAgree.click();
...
//		driver.get(LogoutURL);
	...
	public void AddItemToTheCart() throws InterruptedException {
		...
		if (LiItem > 0) {
			...
			Assert.assertEquals(ActualResult, ExpectedResult, "fofoo"); // the same org.testng
			...
			Assert.assertEquals(ActualValueForCheckOut, ExpectedValueForCheckOut, "fofo hi");
